...
\begin{itemize}
\item \emph{Action} (class) -- integrace s běhovou platformou, vstupní/aktivační bod systému
\item \emph{ValidationTask} (class) -- řízení procesu validace
...
\item \emph{ConfigurationManager} (class) -- komponenta zprostředkující konfigurační uživatelské volby jádru programu
...
\item \emph{OperatorIface} (interface) -- rozhraní, které musí implementovat poskytovatel balíčku operátorů
...
\item \emph{AnalysisIface} (interface) --  rozhraní, které musí implementovat poskytovatel analýzy nad některým z grafů
\item \emph{GraphGeneratorRegister} (class) -- třída poskytující přístup k informacím o existujících poskytovatelích generátorů grafu
\item \emph{OperatorRegister} (class) -- třída poskytující přístup k informacím o existujících poskytovatelích operátorů
\item \emph{AnalysesRegister} (class) -- třída poskytující přístup k informacím o existujících poskytovatelích komponent analýzy
\item \emph{ReportGeneratorRegister} (class) -- třída poskytující přístup k informacím o existujících poskytovatelích generátorů výstupních reportů
\end{itemize}

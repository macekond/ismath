...
Většina majoritních integrovaných prostředí pro vývoj v jazyce Java (IntelliJ Idea, Eclipse, NetBeans) provádí rozsáhlou kontrolu zdrojových kódů přímo během psaní. Programátor je upozořňován na to, že některá metoda nebo třída není definována nebo je definována s jinými parametry. A to v případě, že daná třída se nachází ve zcela jiné kompilační jednotce.
...
Platforma Eclipse poskytuje rozhraní, které reprezentuje java projekt jako DOM prostřednitvím balíčku \verb+org.eclipse.jdt.core.dom+ \cite{parsertools:eclipsejdt}. Poskytovaný AST je možné procházet i modifikovat.
...
Výhodou použití některé z existujích platforem je navíc možnost přímé integrace do vývojového prostředí. Nevýhodou je ovšem svázanost s tímto prostředím. To se však dá zčásti řešit vhodným návrhem jádra systému, keré může být realizováno tak, aby bylo na platformě zcela nezávislé a závislost na platformě byla přidávána až integrací do konkrétního prostředí.
